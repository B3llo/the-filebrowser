            ...
            <table v-if="users.length > 0">
              ...
              <tbody>
                <tr v-for="user in users" :key="user.id">
                  <td>{{ user.username }}</td>
                  <td>
                    <svg v-if="user.perm.admin" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:18px;height:18px">
                      <polyline points="20 6 9 17 4 12"/>
                    </svg>
                    <svg v-else viewBox="0 0 24 24" fill="none" stroke="var(--faint)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:18px;height:18px">
                  ...
                  <td>{{ user.scope }}</td>
                  <td>
                    <router-link :to="'/settings/users/' + user.id" class="fb-settings-table-link">
